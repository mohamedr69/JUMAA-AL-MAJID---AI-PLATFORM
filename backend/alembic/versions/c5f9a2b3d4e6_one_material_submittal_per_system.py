"""One material submittal per system, with its revisions

A system has one material submittal, revised R0, R1, ... until approved.
The register kept a row per form reference, so our own copy of a revision
("MS EML R0.pdf", under review) and the copy the consultant answered
(BBY006-GME-MAS-EL-LI-0001, approved) were two submittals, and a fire rated
cable resubmitted from another supplier was a third: EP-30784 counted seven
submittals for three systems.
  ...
  * The register rows of one system are merged into one: its revisions
    come from all of them (the answered copy of a revision over one still
    under review, the other references kept as "also filed as"), their
    events move onto the row that stays, and a partial unique index then
    refuses a second submittal for a system.
...
ONE_PER_SYSTEM = "system_code IS NOT NULL"
...
def upgrade() -> None:
    ...
    groups: dict = {}
    for row in rows:
        key = (row["project_id"], row["system_code"]) if row["system_code"] else ("row", row["id"])
    ...
    for key, members in groups.items():
        ...
        connection.execute(sa.text(
            f"UPDATE project_submittal_events SET submittal_id = :keeper WHERE submittal_id IN ({', '.join(map(str, ids))})"),
            {"keeper": keeper["id"]})
        ...
        connection.execute(sa.text(
            ...
            {"id": keeper["id"], "at": now,
             "detail": (f"One material submittal for {key[1]}: the register row{'s' if len(merged) > 1 else ''} "
                        f"{described} {'were' if len(merged) > 1 else 'was'} the same submittal, now kept in its revisions")})
        connection.execute(sa.text(f"DELETE FROM project_submittals WHERE id IN ({', '.join(map(str, ids))})"))

    with op.batch_alter_table('project_submittals', schema=None) as batch_op:
        batch_op.create_index('uq_project_submittals_one_per_system', ['project_id', 'system_code'], unique=True,
                              sqlite_where=sa.text(ONE_PER_SYSTEM), postgresql_where=sa.text(ONE_PER_SYSTEM))


def downgrade() -> None:
    """Downgrade schema. The merged register rows are not split again."""
    with op.batch_alter_table('project_submittals', schema=None) as batch_op:
        batch_op.drop_index('uq_project_submittals_one_per_system',
                            sqlite_where=sa.text(ONE_PER_SYSTEM), postgresql_where=sa.text(ONE_PER_SYSTEM))
