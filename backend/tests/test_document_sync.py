...
def test_file_sync_puts_each_file_in_one_status_with_its_reason(client, db_session, tmp_path):
    """File Sync's categories, read off the index: an error, an online-only
    file, a file read in part, one read by the last sync, one left as it
    was -- and a file gone from the folder is not counted at all."""
    from datetime import timedelta

    from app.core.timeutils import utc_now
    from app.models import BackgroundJob

    folder = tmp_path / "EP-30831"
    project_id = _project(client, folder, "30831")
    started = utc_now() - timedelta(minutes=10)
    finished = started + timedelta(minutes=2, seconds=5)
    db_session.add(BackgroundJob(project_id=project_id, kind="sync_documents", status="succeeded", progress={},
                                 created_by_id=1, started_at=started, finished_at=finished, result={"removed": 1}))

    def row(name, **fields):
        db_session.add(ProjectDocument(project_id=project_id, role="document", path=str(folder / name),
                                       relative_path=f"05- Drawings/{name}", filename=name,
                                       **{"state": "fresh", **fields}))

    row("broken.pdf", state="failed", error="SyncError: the model gave no reading of the form")
    row("cloud.pdf", extracted={"records": [], "notes": [
        "cloud.pdf is not downloaded from OneDrive; make the project folder available offline, then refresh."]})
    row("long.pdf", last_processed_at=started + timedelta(seconds=30), extracted={"records": [], "notes": [
        "long.pdf: only the first 12 pages were checked for submission replies."]})
    row("new.pdf", last_processed_at=started + timedelta(seconds=40))
    row("old.pdf", last_processed_at=started - timedelta(days=3))
    row("gone.pdf", state="removed")
    db_session.commit()
    ...
    files = {f["name"]: f for f in client.get(f"/projects/{project_id}/documents/sync-files").json()}
    assert set(files) == {"broken.pdf", "cloud.pdf", "long.pdf", "new.pdf", "old.pdf"}
    assert files["broken.pdf"]["reason"] == "SyncError: the model gave no reading of the form"
    assert files["cloud.pdf"]["reason"] == "File is online-only in OneDrive and could not be processed."
    assert files["long.pdf"]["status"] == "partial" and "first 12 pages" in files["long.pdf"]["reason"]
    assert (files["new.pdf"]["status"], files["old.pdf"]["status"]) == ("processed", "unchanged")
    only = client.get(f"/projects/{project_id}/documents/sync-files", params={"status": "unavailable"}).json()
    assert [f["name"] for f in only] == ["cloud.pdf"]
    assert client.get(f"/projects/{project_id}/documents/sync-files", params={"status": "odd"}).status_code == 400


def test_a_file_onedrive_had_not_brought_down_is_read_on_the_next_sync(client, db_session, tmp_path, ai):
    """Making the folder available offline changes neither a file's size nor
    its time, so a sync that went by those alone never read it again."""
    folder = tmp_path / "EP-30832"
    _pdf(folder / "05- Drawings" / "L01.pdf", _DRAWINGS["05- Drawings/L01.pdf"])
    project_id = _project(client, folder, "30832")
    assert client.post(f"/projects/{project_id}/jobs/sync-documents").json()["status"] == "succeeded"
    row = db_session.query(ProjectDocument).filter(ProjectDocument.project_id == project_id,
                                                   ProjectDocument.relative_path == "05- Drawings/L01.pdf").one()
    # As the first sync would have left it had the file been online-only.
    row.extracted = {"records": [], "notes": [
        "L01.pdf is not downloaded from OneDrive; make the project folder available offline, then refresh."]}
    db_session.commit()
    ...
    counts = client.get(f"/projects/{project_id}/documents/sync-summary").json()["counts"]
    assert counts["unavailable"] == 0 and counts["processed"] == 1
