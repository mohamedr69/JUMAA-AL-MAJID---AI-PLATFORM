...
Beside the specification sit **AI Quick Actions** -- preparing the compliance statement clause by clause from the project's submittals and calculations, and checking what is submitted against what the specification asks -- listed so the intent is visible and **marked "soon"**: they are disabled, because nothing should look like it has been checked when it has not.
...
Logs includes ALL and per-system views with revision history, search, status filters, file links and CSV export. Edwards FAS, VE and Fire Telephone share FAS; monitored self-contained lighting shares EML. Full Package scope includes Fire Rated Cable with Material Submittals only. Material Submittals shows only register entries with a recorded consultant reply. Drawings and Samples refresh from the project directory; filename-derived revisions and missing metadata are identified explicitly. Technical Queries is removed from navigation.
